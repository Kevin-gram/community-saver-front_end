...
const FinancialReports: React.FC<FinancialReportsProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      <div className="relative bg-white rounded-xl shadow-2xl border border-gold-200 w-[500px] max-h-[600px] z-60 flex flex-col">
        <div className="flex items-center justify-between p-6 border-b border-gold-300 bg-gradient-to-r from-gold-50 to-gold-100">
          <div className="flex items-center">
            <FileDown className="w-6 h-6 text-gold-600 mr-2" />
        ...
        <div className="p-6 flex-1 overflow-hidden flex flex-col">
          ...
          {loading ? (
            <div className="flex items-center justify-center py-12">
              ...
              <span className="ml-2 text-gold-700 font-medium">
                {t("financialReports.loading")}
              </span>
            </div>
          ) : error ? (
            <div className="bg-gold-50 border border-gold-200 rounded-lg p-4 text-gold-700 text-sm">
              {error}
            </div>
          ) : reports.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-12 text-center">
              <FileDown className="w-12 h-12 text-gold-300 mb-3" />
                ...
                return (
                  <div
                    ...
                    className={`relative border rounded-lg px-4 py-3 transition-all duration-200 ${
                      isLatest
                        ? "bg-gradient-to-r from-gold-100 to-gold-50 border-gold-300 shadow-sm"
                        : "bg-gold-50 border-gold-200 hover:bg-gold-100"
                    }`}
                  >
                    {isLatest && (
                      <div className="absolute -top-2 -right-2 bg-gold-600 text-white text-xs font-semibold px-2 py-1 rounded-full flex items-center shadow-md">
                    ...
                    <div className="flex items-start justify-between gap-3">
                      <div className="flex-1 min-w-0">
                        <div className="font-semibold text-gold-900 mb-1 truncate">
                          {report.description ||
                            report.fileName ||
                            "Untitled Report"}
                        </div>
                        {report.period && (
                          <div className="text-xs text-gold-700 mb-1">
                            {t("financialReports.period")}: {report.period}
                          </div>
                        )}
                        <div className="text-xs text-gold-600 flex items-center">
                      ...
                      <button
                        ...
                        className={`flex items-center space-x-2 px-4 py-2 rounded-lg font-medium text-sm transition-all duration-200 whitespace-nowrap ${
                          downloadingId === report._id
                            ...
                            : "bg-gold-600 text-white hover:bg-gold-700 hover:shadow-md"
                        }`}
                        aria-label={`Download ${
